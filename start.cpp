...
bool check_five(int x,int y,int c){
	...
	if (ry-ly>=4) return 1;
	lx=rx=x,ly=ry=y;
	while (lx>0&&data[lx-1][y]==c&&ly>0&&data[x][ly-1]==c) --lx,--ly;
	while (rx<N-1&&data[rx+1][y]==c&&ry<N-1&&data[x][ry+1]==c) ++rx,++ry;
	if (rx-lx>=4) return 1;
	lx=rx=x,ly=ry=y;
	while (lx>0&&data[lx-1][y]==c&&ry<N-1&&data[x][ry+1]==c) --lx,++ry;
	while (rx<N-1&&data[rx+1][y]==c&&ly>0&&data[x][ly-1]==c) ++rx,--ly;
	if (rx-lx>=4) return 1;
	return 0;
}
void print(){
